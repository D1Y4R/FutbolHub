...
def get_fixtures():
    try:
        ...
        # Call the actual function to get fixtures
...
def get_football_data_fixtures(date):
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if isinstance(data, dict) and 'response' in data and len(data['response']) > 0:
                # New API already returns in standard format
                return jsonify(data)
...
def get_team_stats(team_id):
    ...
    try:
        ...
        for match in matches:
            ...
            if fixture_date and str(current_year) in fixture_date:
        ...
        # Format last 10 matches
        formatted_matches = []
        for match in filtered_matches[:10]:  # Last 10 current matches
            ...
            try:
                ...
                date_obj = datetime.strptime(fixture_date.split('T')[0], '%Y-%m-%d')
...
def get_team_stats_api(team_id):
    ...
    try:
        ...
        for match in filtered_matches:  # Tüm güncel maçları kontrol et
            # Get match status
            match_status = match.get('fixture', {}).get('status', {}).get('short', '').strip()
            
            # Skip unfinished or ongoing matches
