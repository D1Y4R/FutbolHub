...
class AsyncDataFetcher:
    ...
    async def fetch_team_data_batch(self, team_ids, api_key):
        """Fetch team data in batch using new API"""
        base_url = "https://v3.football.api-sports.io/teams"
        headers = {'x-apisports-key': api_key}
        
        # For new API, create URLs and params for each team
    ...
    async def fetch_matches_parallel(self, league_ids, date, api_key):
        """Fetch league matches in parallel using new API"""
        base_url = "https://v3.football.api-sports.io/fixtures"
        headers = {'x-apisports-key': api_key}
        
        # Params for each league
        ...
        for league_id, result in zip(league_ids, results):
            if result['status'] == 'success':
                ...
                # New API returns matches in 'response' field
                matches = data.get('response', []) if isinstance(data, dict) else []
                if isinstance(matches, list):
                    for match in matches:
                        match['league_id'] = league_id  # Add league ID
    ...
    async def fetch_h2h_data(self, home_team_id, away_team_id, api_key, home_team_name=None, away_team_name=None):
        """Fetch head-to-head data using new API"""
        url = "https://v3.football.api-sports.io/fixtures/headtohead"
        headers = {'x-apisports-key': api_key}
        
        # New API uses h2h parameter with team IDs separated by dash
        params = {
        ...
        result = await self.fetch_single(url, params, headers)
        
        if result['status'] == 'success' and result.get('data'):
            h2h_data = result['data']
            # New API returns data in 'response' field
            matches = h2h_data.get('response', [])
            if isinstance(matches, list) and matches:
                logger.info(f"H2H data fetched successfully: {len(matches)} matches found")
